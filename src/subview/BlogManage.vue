...
api.get(getCountyList).then((result) => {
  country.value = result.data.data
})

api.get(getTypeList).then((result) => {
  ...
  options: {
    upload: (file) => {
      return new Promise((resolve, reject) => {
        ...
        api
          .post(fileUploading, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
          })
          .then((res) => {
            console.log(res)
            let newUrl =
              'http://124.222.113.82:8999/file/download/' + res.data.data
            resolve(newUrl)
          })
          .catch((err) => {
        ...
        <el-form :model="blogAdd" label-width="auto" style="max-width: 800px">
          ...
          <el-form-item label="封面图片"
            ><el-upload
              ...
              action="http://124.222.113.82:8999/file/upload"
              :show-file-list="false"
              :on-success="handleAvatarSuccess"
            >
              <img v-if="imageUrl" :src="imageUrl" class="avatar" />
              <el-button plain type="primary" v-else>点击上传</el-button>
          ...
          <QuillEditor
            ...
            :modules="modules"
            toolbar="full"
            style="height: 600px; margin-bottom: 12px"
          />

          <!--          <QuillEditor2 v-model="blogAdd.text" />-->
          <!--          <quill-editor  theme="snow" toolbar="full"/>-->
          <el-button @click="dialogVisible = false">取消</el-button>
          <el-button type="primary" @click="blogSubmit"> 提交文章 </el-button>
        </el-form>
      </template>
    </el-dialog>
